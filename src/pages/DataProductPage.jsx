          ...
          <button
            onClick={(e) => {
              ...
              if (data.onOpenSettings) {
                ...
                alert(
                  "Error: Handler not available. Please try refreshing the page."
                );
              }
            }}
            style={{
        ...
        {data.fields.length === 0 ? (
          ...
          data.fields.map((field, idx) => (
            <div
              ...
              onMouseLeave={(e) => {
                // e.currentTarget.style.background = field.calculation !== null ? '#8cbdeeff' : 'white';
                if (!field.isPK) {
...
const DataProductPage = () => {
  ...
  useEffect(() => {
    console.log("1.Canvas nodes:", nodes);
  }, [nodes]);

  useEffect(() => {
    console.log("1.Canvas edges:", edges);
  }, [edges]);

  useEffect(() => {
       console.log("1.Table metadata:", tableMetadata);
  }, [tableMetadata]);
  ...
  const loadDataProduct = (dataProduct) => {
    try {
      ...
      // Handle entities as array (your format)
      const entitiesArray = Array.isArray(dataProduct.entities)
        ? dataProduct.entities
        : Object.values(dataProduct.entities || {});

      // Build entity key mapping using existing IDs
      const entityKeyMap = new Map(); // nodeId -> entityKey
      ...
      // Create nodes from canvas entities array - KEEP ORIGINAL IDs
      entitiesArray.forEach((canvasNode) => {
        const nodeId = canvasNode.id; // Use existing ID
        const entityKey = `${canvasNode.data.tableType}_${canvasNode.data.tableName}`;
      ...
      // Create edges from relationships - use IDs directly
    ...
    setNodes((nds) =>
      nds.map((node) => {
        if (node.id === addFieldNodeId) {
          ...
          // console.log("In handleaddconformfield(),Updated metadata for node:", updatedNode);
          return updatedNode;
        }
        return node;
      })
  ...
  // useEffect(() => {
  //     console.log("Current Custom tables:", customTables);
  // }, [customTables]);
  ...
  const handleSave = async () => {
    try {
      ...
      console.log("Relationships to save:", relationships);
  ...
  const handleShowReverseDeps = async (nodeId, tableName, tableType) => {
    try {
          ...
          if (refs && Array.isArray(refs)) {
            refs.forEach((refPath) => {
              ...
              if (refEntity && refField && refEntity !== entityKey) {
                // Skip if entity doesn't exist in metadata
      ...
      console.log("Found downstream dependencies:", foundReverseDeps);
  ...
  const handleAddSuggestedEntity = async (suggestion) => {
    try {
      ...
      const entity =
        suggestion.entityData || sourceEntities[suggestion.entityName];
      ...
      // Extract fields from entity - handle both array and object formats
      ...
      setSourceDataProduct((prev) => {
        ...
        updatedEntities[suggestion.entityName] =
          suggestion.entityData || entity;
      ...
      setTimeout(() => {
        ...
        if (suggestion.dependencyMap) {
          ...
          Object.keys(suggestion.dependencyMap).forEach(
            (dependentEntityKey) => {
            ...
            setTimeout(() => {
              setNodes((currentNodes) => {
                const { nodes: layoutedNodes, edges: layoutedEdges } =
                  getLayoutedElements(currentNodes, updatedEdges, "LR");
                setEdges(layoutedEdges);
  ...
  const handleAddReverseDep = async (suggestion) => {
    try {
      ...
      console.log("Adding reverse dependency for suggestion:", tableMetadata);
      ...
      console.log("Found entity data:", entity);
      // Extract fields from entity - handle both array and object formats
      const entityFields = Array.isArray(entity.fields)
        ? entity.fields.map((f) => ({
            name: f.name,
      ...
      // Update tableMetadata to include this entity for future dependency lookups
      setTableMetadata((prev) => ({
        ...prev,
        ...
        Object.keys(suggestion.dependencyMap).forEach((dependentEntityKey) => {
          ...
          connections.forEach((conn) => {
            ...
            if (hasSourceField && hasTargetField) {
              ...
              newEdges.push({
                id: `e-${newNodeId}-${targetNode.id}-${conn.sourceField}-${
                  conn.targetField
      ...
      setTimeout(() => {
        setNodes((currentNodes) => {
          setEdges((currentEdges) => {
            const { nodes: layoutedNodes, edges: layoutedEdges } =
              getLayoutedElements(currentNodes, currentEdges, "LR");
            setNodes(layoutedNodes);
            setEdges(layoutedEdges);
            return currentEdges;
          });
  ...
  const checkIfEntityCanBeFormed = (
    entityKey,
    entityFields,
    entity,
    canvasTables
    ...
    entityFields.forEach((fieldName) => {
      ...
      if (fieldData.ref && Array.isArray(fieldData.ref)) {
        ...
        fieldData.ref.forEach((refPath) => {
          ...
          const refType = refEntity.match(/^(BASE|CTE|VIEW)_/)
            ? refEntity.match(/^(BASE|CTE|VIEW)_/)[1]
            : "BASE";

          // Check if this table exists in canvas (match by name or full key)
          const canvasTable = canvasTables.find(
            (t) =>
              t.name === refTableName ||
              t.name === refEntity ||
              t.fullKey === refEntity
      ...
      // Check if field has calculation
        ...
        if (
          fieldData.calculation.ref &&
          Array.isArray(fieldData.calculation.ref)
        ) {
          ...
          if (allRefsFound) {
            ...
            fieldSources[fieldName].push(
              `Calc: ${fieldData.calculation.expression}`
            );
          }
        }
      }
    ...
    const coveragePercent = Math.round(
      (foundFields.size / entityFields.length) * 100
    );
          ...
          >
            ...
            >
              {currentDataProductName
                ? currentDataProductName.replace(".json", "")
                : "New Data Product"}
            </h1>
            <div
              style={{
          ...
          <button
            onClick={() => {
              ...
              edges.forEach((edge) => {
                ...
                if (sourceNode && targetNode) {
                  const sourceFieldName = edge.sourceHandle?.replace(
                    "-source",
                    ""
                  );
                  ...
                  const table = {
                    ...
                    fields: node.data.fields.map((field) => {
                      ...
                      if (fieldEdgeMap[fieldKey]) {
                        ...
                        if (
                          !fieldObj.calculation &&
                          fieldEdgeMap[fieldKey].calculation
                        ) {
                          fieldObj.calculation =
                            fieldEdgeMap[fieldKey].calculation;
                        }

                        fieldObj.relationships =
                          fieldEdgeMap[fieldKey].relationships;
                      }
          ...
          >
            ...
            >
              ...
              >
                In canvas,
                <br />
                Toggle OFF = <strong>{globalAttributeMode}</strong>
                <br />
                Toggle ON ={" "}
                <strong>
            ...
            >
              <button
                ...
                style={{
                  padding: "12px 16px",
                  backgroundColor:
                    settingsActiveTab === "byMode" ? "white" : "transparent",
                  border: "none",
                  borderBottom:
              ...
              <button
                ...
                style={{
                  padding: "12px 16px",
                  backgroundColor:
                    settingsActiveTab === "selected" ? "white" : "transparent",
                  border: "none",
                  borderBottom:
                  ...
                  color:
                    settingsActiveTab === "selected" ? "#6366f1" : "#6b7280",
                }}
              >
                Selected Attributes
              </button>
              ...
              {settingsActiveTab === "byMode" ? (
                ...
                <>
                  ...
                  <div>
                    ...
                    >
                      {tab1FilterMode.charAt(0).toUpperCase() +
                        tab1FilterMode.slice(1)}{" "}
                      Mode Attributes (
                      {settingsData.allFields
                        ? tab1FilterMode === "both"
                          ? settingsData.allFields.length
                          : settingsData.allFields.filter((field) => {
                              const toggleKey = `${settingsData.nodeId}_${field.name}`;
                              const isToggled =
                                attributeToggles[toggleKey] || false;
                              const effectiveMode = isToggled
                                ? globalAttributeMode === "runtime"
                                  ? "loadtime"
                                  : "runtime"
                                : globalAttributeMode;
                    ...
                    >
                      {settingsData.allFields &&
                      settingsData.allFields.length > 0 ? (
                        settingsData.allFields
                          .filter((field) => {
                      ...
                      {settingsData.allFields &&
                        ...
                        settingsData.allFields.filter((field) => {
                          ...
                          >
                            No {tab1FilterMode} attributes. Toggle attributes to
                            change their mode.
                          </p>
                        )}
                    </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    settingsData.allFields.filter(
                      ...
                      >
                        {settingsData.allFields
                          .filter((f) => attributeSelections[f.name])
                          .map((field, idx) => (
                            <span
                              key={idx}
                              style={{
                    ...
                    >
                      ...
                      settingsData.allFields.length > 0 ? (
                        settingsData.allFields
                          ...
                          .map((field, idx) => {
                            const isSelected =
                              attributeSelections[field.name] || false;
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  color: "#1f2937",
                                  borderBottom:
                                    idx < settingsData.allFields.length - 1
                                      ? "1px solid #e5e7eb"
                                      : "none",
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  ...
                  <strong>"{deleteFieldInfo.fieldName}"</strong>? This will also
                  delete all connections to this field. This action cannot be
                  undone.
                </p>
              </div>
            </div>
        ...
        <>
          ...
          >
            ...
            <div style={{ marginBottom: "24px" }}>
              ...
              >
                Define how this attribute is calculated from the source
                attribute
              </div>
            </div>
            <div
